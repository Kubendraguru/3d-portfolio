...
.${NS}-small {
  transform: scale(1.06) translate(-4px, -4px) translateZ(8px);
}
.${NS}-big {
  transform: scale(1.18) translate(-14px, -14px) translateZ(24px);
...
@keyframes ${NS}-glow {
  ...
  to  { filter: drop-shadow(0 4px var(--ag-glow-blur) var(--ag-glow-end)); }
}
@keyframes ${NS}-glow-small {
  0%  { filter: drop-shadow(0 0 2px var(--ag-glow-start)); }
  to  { filter: drop-shadow(0 2px var(--ag-glow-blur-small) var(--ag-glow-start)); }
}
`;
...
export const InteractiveGrid: React.FC<InteractiveGridProps> = (props) => {
  const {
  ...
  return (
    <div
      className="w-full max-w-7xl mx-auto px-2 sm:px-4 md:px-8 py-6 sm:py-10 select-none"
      style={
        {
          ...style,
          position: 'relative',
    ...
    >
      <style>{CSS}</style>
      <div
        className="w-full rounded-[32px] sm:rounded-[40px] bg-[#0E0F12] border border-[#22242A] shadow-[0_30px_90px_rgba(0,0,0,0.9)] overflow-hidden"
        style={{ padding }}
      >
        <div
