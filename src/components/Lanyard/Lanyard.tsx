...
function Band({
  ...
  const cardMap = useMemo(() => {
    ...
    const drawFitted = (img: HTMLImageElement | any, rect: typeof FRONT_UV_RECT) => {
      const rx = rect.x * W;
      const ry = rect.y * H;
      const rw = rect.w * W;
      const rh = rect.h * H;
      ...
      const dw = (img.width || rw) * scale;
      const dh = (img.height || rh) * scale;
      const dx = rx + (rw - dw) / 2;
      const dy = ry + (rh - dh) / 2;
      ctx.save();
      ctx.beginPath();
  ...
  // Exact React Bits joints: segments hang straight and connect precisely to top clamp
